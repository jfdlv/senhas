...
<hr>
	...
	<form method="post" action="cont_test1.php" class="form">
					<div class="row">
				    <?php 
				    		$con = 0;
				    		foreach ($pregs as $preg) {
				    				...
				    				echo "<div class='row'>";
										echo "<div class='col-md-12 text-center'>";
											echo "<div class='row'><h3>".$preg['desc']."</h3></div>";	
													$p = new pregunta($preg['id'],$preg['desc']);
													$resps = $p -> obtenerRespuestas();
													foreach ($resps as $resp) {
													echo "<div class='row'>";
													$al = array();
													$val = $p -> obtenerRespuestaA();
													for ($i=0; $i < 3; $i++) { array_push($al, $p -> obtenerRespuestaA()); }
													array_push($al, $resp);
													shuffle($al);
													$idr=$resp['idr'];
													echo "<div class='row text-center'>";
													echo "<div class='col-md-12 col-md-offset-2'>";
													foreach ($al as $a) {
														echo "<div class='col-md-1'><input type='radio' name='group".$idr."' value='".$a['id']."' required></div>";
														echo "<div class='col-md-1'><img class='img-responsive'src='".$a['imagen']."' id='imagen".$con."' onload=\"cortar('#imagen".$con."')\"></div>";
														$con++;
													}
													echo "</div>";
													echo "</div>";
